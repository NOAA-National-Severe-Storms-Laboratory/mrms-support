...
/*------------------------------------------------------------------
	...
	Purpose:	Compute epoch seconds for a given time	
				
	Input:		tim = pointer to struct tm 
					
	Output:		time_t storing epoch seconds
...

{
    ...
    gm_time->tm_mday = day;
    gm_time->tm_mon  = mo-1;
    gm_time->tm_year = yr-1900;
    gm_time->tm_isdst = -1;

    epoch_seconds = (long)make_time( gm_time );
